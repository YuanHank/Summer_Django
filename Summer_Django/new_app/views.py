...
import numpy as np
import string
from .models import Hw1Improve,User,Wormbase285,WormbaseGenetranscriptW285,TranscriptWbidType,FinalResultWt1HrdeipWs285AllWithIdtype,Clash
import json
from .check_duplicate import check_duplicate
import re
from .wormbase import wormbase_crawler,wormbase_searching
from .enrichment_website import enrichment_website,enrichment_compare
...
def enrichment_click_table(request,pk):
    pk = pk.split("_")
    domain = pk[0]
    #input_list = pk[1]
    return render(request,'enrichment_test.html',locals())

def enrichment_check(request):
    url = request.POST['url'] 
    url = url.split('_')
    domain = url[0]
    input_list = url[1].split(',')
    domain_list,result = enrichment_compare(input_list,domain)
...
def enrichment_ajax(request):
    ...
    #IDs_table = result[['Domain_id','IDs']].copy()
    ...
    '''
    表格作法想法：是否可以把現有表格的Domain_id值改成button或者是一個link，點下去之後會執行ajax，並將value設定成該文字，
    在傳ajax時候會執行對照，比對出input以及點選的domain_id的重複點，並以此為基礎製作表格並顯現（表格原本就存在，但無內容物品且隱藏，點選後才會顯現以及附加內容物）
    '''
'''==========================================user(暑假的最初練習)=================================================='''
